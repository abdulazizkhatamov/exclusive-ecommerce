...
import { toggleUpdateVariantSheet } from "@/features/ui/ui-variant-slice.ts";
import { IProduct, IAttribute } from "@/types/product.ts";
import {
  Select,
  SelectTrigger,
  SelectValue,
...
const UpdateVariantForm: React.FC<UpdateVariantSheetProps> = ({ product }) => {
  const dispatch = useDispatch();
  const { updateVariantSheet } = useSelector(
  ...
  useEffect(() => {
    ...
    if (updateVariantSheet.variant?.images) {
      setOldImages(updateVariantSheet.variant.images);
    }
  }, [updateVariantSheet.variant]);

  const updateVariantFormik = useFormik({
    initialValues: {
      sku: updateVariantSheet.variant?.sku || "",
      product: product._id,
      attributes: product.attributes.map((attribute) => ({
        name: attribute.name,
        value: attribute.value.length > 0 ? attribute.value[0] : "",
      })),
      price: updateVariantSheet.variant?.price || 0,
      stock: updateVariantSheet.variant?.stock || 0,
    },
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      updateVariantMutation.mutate(
        {
          id: updateVariantSheet.variant?._id || "",
          formData: updatedFormData,
        },
        {
          onSuccess: async () => {
            await queryClient.invalidateQueries("variants");
            dispatch(toggleUpdateVariantSheet({ variant: null }));
            toast({
  ...
  useEffect(() => {
    // Set old images when the sheet is opened
    if (updateVariantSheet.variant) {
      updateVariantFormik.setFieldValue("sku", updateVariantSheet.variant.sku);
      updateVariantFormik.setFieldValue(
        "price",
        updateVariantSheet.variant.price,
      );
      updateVariantFormik.setFieldValue(
        "stock",
        updateVariantSheet.variant.stock,
      );
    }
  }, [updateVariantSheet.variant]);
  ...
  return (
    <Sheet
      open={updateVariantSheet.sheet}
      onOpenChange={() => dispatch(toggleUpdateVariantSheet({ variant: null }))}
    >
      <SheetContent>
        <SheetHeader>
        ...
        >
          <div className="grid w-full max-w-sm items-center gap-1.5">
            ...
            <Input
              ...
              autoComplete={"off"}
            />
            {updateVariantFormik.touched.sku &&
          ...
          <div className="grid w-full max-w-sm items-center gap-1.5">
            ...
            <div className="relative">
              <Button
                ...
                onClick={() =>
                  document
                    .getElementById("update-variant-images-input")
                    ?.click()
                }
