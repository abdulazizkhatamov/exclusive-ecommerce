...
import { toggleCreateVariantSheet } from "@/features/ui/ui-variant-slice.ts";
...
import { Trash } from "lucide-react"; // Assuming Select component is from ShadCN
...
const CreateVariantForm: React.FC<CreateVariantSheetProps> = ({ product }) => {
  const dispatch = useDispatch();
  const { createVariantSheet } = useSelector(
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      createVariantMutation.mutate(formData, {
        onSuccess: async () => {
          ...
          dispatch(toggleCreateVariantSheet());
          toast({
  ...
  return (
    <Sheet
      ...
      onOpenChange={() => dispatch(toggleCreateVariantSheet())}
    >
      <SheetContent>
        <SheetHeader>
        ...
        >
          <div className="grid w-full max-w-sm items-center gap-1.5">
            ...
            <Input
              ...
              autoComplete={"off"}
            />
            {createVariantFormik.touched.sku &&
